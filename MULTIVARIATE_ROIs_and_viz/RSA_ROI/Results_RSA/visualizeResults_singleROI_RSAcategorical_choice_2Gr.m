clear all
load('Mask_RSACateg5_CAT>CB2_TFCE05FWE_MASK-FGall_dsmPartialSpearmanCorr_CAT5_HmaxC1_GROUPSlab_iter_0.mat');
max_corr_path=fullfile(cd,'Intra_Corr_results/');
max_corr_name='FGall_CAT>CB2_CAT5_realValues_IntraGr';
...
%excluding subjects
ParR_targetCategoricalCAT(10)=[];%Excluding 'NaAs'
ParR_targetCategoricalCON(16)=[];%Excluding 'BrCh'
ParR_targetCategoricalC1B(10)=[];%Excluding 'JiFe'
ParR_targetCategoricalC2B(10)=[];%Excluding 'Jife'

% if Group_combo==1
    dataCAT=ParR_targetCategoricalCAT;

    dataCON=ParR_targetCategoricalCON;
    
    dataC1B=ParR_targetCategoricalC1B;
    
    dataC2B=ParR_targetCategoricalC2B;
% else
%     dataCAT=ParR_targetCategoricalCAT';
% 
%     dataCON=ParR_targetCategoricalCON';
...
for i_col=1:length(XTick)
    ...
    % title ('RSA categorical model FG-all');
